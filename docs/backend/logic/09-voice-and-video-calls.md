...
```
Caller                     Signaling (doc 07 WS)                    Callee
  │  call.invite ─────────────────►│                                  │
  │  (conversationId)              │ 1. verify membership (doc 07)    │
  │                                │ 2. check block / policy (doc 07) │
...
- **Signaling** = the doc 07 WebSocket gateway carries call setup messages (invite, accept, SDP, ICE candidates, hangup). No new socket infrastructure.
...
The conversation header (doc 07) exposes two buttons, already present in the UI: a phone icon (voice) and a video icon (video).
...
- Buttons are disabled if a call between these users is already active, if either party has blocked the other (doc 07 §9), or if the relationship policy forbids contact.
...
- **`CallGateway`** extends the doc 07 socket connection — no separate handshake. Call events validate conversation membership the same way message events do.
...
Every terminal call writes a **system message** into the chat conversation (doc 07 `type: system`) — e.g. "Video call · 4:12" or "Missed call" — so call history lives inline with the chat.

---

## 5. Signaling events & APIs

### WebSocket signaling (via doc 07 gateway)
...
| `conversation_id` | uuid FK              | The direct conversation (doc 07)                         |
...
Inherits the doc 07 safety layer — calls are not a bypass around it.

- **Blocking:** a blocked user cannot call, and their invite never rings the other side. Enforced server-side on `call.invite`.
- **Relationship policy:** the same "who can contact whom" gate as DMs (doc 07 §9) applies to calls. Stranger calls are off by default; minor accounts get the strictest policy and may have calling disabled entirely.
...
- **Reporting:** a call can be reported after the fact (reuses doc 07 `chat_reports` with a `call` context); abusive calling patterns are rate-limited.
...
- Blocks and the relationship policy are enforced on `call.invite`, mirroring doc 07 §9.
...
- [ ] Signaling rides the existing doc 07 WebSocket gateway — no new socket handshake
