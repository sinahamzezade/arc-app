...
**Downstream:** [03 — Skill Graph, Roadmap Generator & AI Coach](./03-goals-and-roadmap.md)
...
/onboarding → /questionnaire → GET schema + GET answers
  ...
  → /questionnaire/review → POST submit → Roadmap Generator job → /home
```
...
| `ROADMAP_GENERATION_FAILED` | Sync failure (prefer async) |
...
1. Strict validate vs schema (**visible steps only**)
2. Upsert response → `submitted`
3. Upsert active `goals` (token bridge)
4. Mark profile completed
5. Enqueue Roadmap Generator stub (`enqueueGenerate(goalId)`)
...
Idempotent: already-submitted returns existing goal + new queued job stub.
