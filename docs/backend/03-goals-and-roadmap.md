...
**Depends on:** [02 — Question Engine](./02-questionnaire.md) (submit upserts `goals` + enqueues generate job)
...
POST /questionnaire/submit  (Question Engine)
  ...
  → enqueue generate_roadmap(goal_id)  → jobId in response
      → Roadmap Generator:
          ...
          = personalized learning path for this user
  → later: AI Coach listens to progress / assessments → patch path instance
```
...
Gate: frontend blocks app routes until questionnaire completed (doc 02). Generator assumes goal already exists. After submit, UI should land on `/path` (or home with generating banner) and poll until path ready.
...
| `difficulty` | varchar | |
| `xp_reward` | int | Default XP (server still awards on complete) |
...
| `status` | varchar | `locked` \| `available` \| `completed` (or derive from progress) |
