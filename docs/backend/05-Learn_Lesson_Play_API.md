...
1. Load lesson + progress; reject if locked / not owned.
2. If already `completed` and rewards already granted → return prior award (idempotent); do not double-pay.
3. Grade practice + quiz server-side.
4. Compute rewards (§5).
5. Set progress `completed`, `completedAt`, `xpAwarded`.
6. Set `lessons.status = completed` for this user’s path model (if status is per-user via progress, update derived status in tree serializer from progress — preferred).
7. Unlock next lesson in milestone / phase (first locked → `available`).
8. Update roadmap `progressPercent`.
9. Credit `profiles.total_xp`, `gems`, `coins`.
10. Unlock badge if eligible (e.g. first completed lesson → `first-step`).
11. Emit streak / notification hooks if those modules exist (daily meaningful action = lesson complete).
...
## 5. Reward rules (align with product specs)

Sources: `Arc_MVP_Full_Technical_Roadmap.md`, `Arc_Battle_XP_Gems_Coins_Referral_Spec.md`.

| Currency | Lesson complete baseline                                   | Notes                                                                           |
| -------- | ---------------------------------------------------------- | ------------------------------------------------------------------------------- |
| XP       | Use `lessons.xp_reward` (typically 20–40)                  | Preview on overview; grant only on `complete`. Repeat complete → reduced / zero |
| Gems     | By difficulty: easy 1–2, medium 3–4, hard 5–7, expert 8–10 | +2 if perfect quiz                                                              |
| Coins    | ~10–15 for standard lesson (UI mock uses 15 / synth 10)    | Server decides; do not trust client                                             |
| Badge    | `first-step` on first ever lesson complete                 | Label: “First Step”                                                             |

`rewardPreview` on GET may show expected first-time award; actual grant on complete may differ (perfect bonus, repeat penalty).

**Never trust client-sent XP/gems/coins amounts.**
...
LessonsModule
  ...
  LessonRewardsService
    compute + apply to Profile (idempotent)
  LessonUnlockService
...
- [ ] Profile `totalXp` / `gems` / `coins` update atomically with progress.
...
- Weekly plan scheduling (separate).
- Battle / league XP split (grant lifetime XP now; weekly league bucket when that module lands).
...
3. **`POST …/complete`** + profile economy + unlock + tree status
